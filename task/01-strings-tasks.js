...
function getRectangleString(width, height) {
  ...
  const minus = '─';
  const stick = '│';
  const firstLine = `${cornerLeftTop}${minus.repeat(width - 2)}${cornerRightTop}`;
  const lastLine = `${cornerLeftBottom}${minus.repeat(width - 2)}${cornerRightBottom}`;
  const middleLine = `${stick}${' '.repeat(width - 2)}${stick}\n`;
  return `${firstLine}\n${middleLine.repeat(height - 2)}${lastLine}\n`;
}
...
function encodeToRot13(str) {
  ...
  return str.split('').map(i => alphabet.indexOf(i) !== -1 ? shifr[alphabet.indexOf(i)] : i).join('');
}
