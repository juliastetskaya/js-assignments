...
function timeSpanToString(startDate, endDate) {
  ...
  const trans = num => num < 10 ? `0${num}` : num;
  return `${trans(hours)}:${trans(minutes)}:${trans(seconds)}.${milliSeconds < 10 ? `00${milliSeconds}` : milliSeconds}`;
}
...
function angleBetweenClockHands(date) {
    ...
    const degrees = (hours + (minutes /60)) * 30 - minutes * 6;
