...
function getPolynom(...args) {
  const args1 = args.reverse();
  return x => args1.reduce((acc, item, index) => acc + item * Math.pow(x, index), 0);
}
...
function memoize(func) {
  return (...args) => {
    ...
    if (!func.memo) {
      ...
      func.memo[newArgs] = result;
      return result;
    } else if (!(newArgs in func.memo)) {
      const result = func(...args);
      func.memo[newArgs] = result;
      return result;
    } else {
      return func.memo[newArgs];
    }
  }
}
...
function logger(func, logFunc) {
  return (...args) => {
    logFunc(`${func.name}(${args.map(i => i instanceof Array ? `[${i.map(it => typeof it === 'string' ? `"${it}"` : it)}]` : i)}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${args.map(i => i instanceof Array ? `[${i.map(it => typeof it === 'string' ? `"${it}"` : it)}]` : i)}) ends`);
    return result;
  }
}
